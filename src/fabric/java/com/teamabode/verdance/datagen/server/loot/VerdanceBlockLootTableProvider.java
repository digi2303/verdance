...
public class VerdanceBlockLootTableProvider extends FabricBlockLootSubProvider {
    private static final float[] NORMAL_LEAVES_STICK_CHANCES = {0.02F, 0.022222223F, 0.025F, 0.033333335F, 0.1F};

    public VerdanceBlockLootTableProvider(FabricPackOutput output, CompletableFuture<HolderLookup.Provider> registries) {
    ...
    public void generate() {
        ...
        cushions();
        shrubs();
        add(VerdanceBlocks.SILKWORM_EGGS, block -> LootTable.lootTable().withPool(
                LootPool.lootPool().when(this.hasSilkTouch()).setRolls(ConstantValue.exactly(1.0f)).add(LootItem.lootTableItem(block))));
    }
    ...
    private void cantaloupe() {
        add(VerdanceBlocks.CANTALOUPE, block -> {
            var enchantments = registries.lookupOrThrow(Registries.ENCHANTMENT);
            var lootItem = LootItem.lootTableItem(VerdanceItems.CANTALOUPE_SLICE).apply(SetItemCountFunction.setCount(UniformGenerator.between(2.0f, 4.0f))).apply(ApplyBonusCount.addUniformBonusCount(enchantments.getOrThrow(Enchantments.FORTUNE))).apply(LimitCount.limitCount(IntRange.upperBound(4)));
